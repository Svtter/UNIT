  ...
  def _load_projection(self, src_proj_weight: torch.Tensor, targ_proj_weight: torch.Tensor):
    if src_proj_weight.shape != targ_proj_weight.shape:
      ...
      src_proj_weight = F.interpolate(
        src_proj_weight, size=(self.patch_size, self.patch_size), mode="bicubic", align_corners=True, antialias=False
      )
  ...
  def _get_pos_embeddings(self, batch_size: int, input_dims: Tuple[int, int]):
    ...
    if self.cpe_mode:
      if self.training:
        ...
        pos_embed = F.interpolate(pos_embed.float(), size=(max_dim, max_dim), align_corners=True, mode="bilinear").to(
          pos_embed.dtype
        )
